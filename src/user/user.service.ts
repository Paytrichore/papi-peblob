...
export class UserService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.userApiUrl = this.configService.get<string>('USER_API_URL', 'http://localhost:3001');
  }
  ...
  async userExists(userId: string): Promise<boolean> {
    try {
      ...
      this.logger.warn(`Erreur lors de la vérification de l'utilisateur ${userId}:`, error);
  ...
  async getUserProfile(userId: string): Promise<UserProfile | null> {
    try {
      ...
      this.logger.error(`Erreur lors de la récupération du profil ${userId}:`, error);
      return null;
    }
  }
   ...
   */
  async notifyUserActivity(userId: string, action: string, details?: any): Promise<void> {
    try {
      ...
      this.logger.warn(`Erreur lors de la notification d'activité pour ${userId}:`, error);
